...
void addMenu(GtkWidget *window) {
  ...
  g_signal_connect(menuItem,"activate",G_CALLBACK(menuExpert),NULL);
  menuItem=gtk_check_menu_item_new_with_mnemonic(_("_Temps illimité"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menuNiveau2),menuItem);
  g_signal_connect(menuItem,"activate",G_CALLBACK(menuTime),NULL);
  gtk_widget_set_sensitive(menuItem,FALSE); // Désactive ce menuItem
}
///////////////////////////////////////////////////////////////////////////////
...
static void startApplication(GtkApplication *app,gpointer data) {
  ...
  gtk_window_set_title(GTK_WINDOW(window),"MasterMind v1.1");
...
int main(int argc,char *argv[]) {
  ...
  GtkApplication *app=gtk_application_new("fr.iutbeziers.menus-11",
